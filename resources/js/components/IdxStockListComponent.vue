    ...
    <div class="container">
                    ...
                    <div class="card-body p-0 overflow-auto">
                        ...
                        <b-table :fields="stockFields" :items="stocks" show-empty>
                            ...
                            <template #cell(current_price)="data">
                                {{ data.item.current_price ? Math.round(Number(data.item.current_price)).toLocaleString() : '-' }}
                            </template>
        ...
        <b-modal v-if="user_id" id="modal-1" hide-footer scrollable size="lg"
            ...
            <b-form @submit.stop.prevent="fetchStockResources">
                ...
                <b-table :fields="stockResourceFields" :items="stockResources" bordered hover small striped>
                    <template #cell(actions)="row">
...
export default {
    ...
    methods: {
        ...
        fetchStocks() {
            ...
            axios.get('/api/watchlist/idx-stocks').then((response) => {
        ...
        fetchStockResources() {
            ...
            axios.get('/api/watchlist/get-resource-idx-stock', {
                params: {
                    query: this.query
                }
            }).then((response) => {
        ...
        store(stock, index) {
            axios.post('/api/watchlist/store-idx-stock', {
                id: stock.id
            }).then((response) => {
                if (response.data.status) {
                    this.$bvToast.toast('Stock has been added to watchlist.', {
                        title: `Indonesia Stock Watchlist`,
                        variant: 'success',
                        solid: true
                    })
        ...
        remove({stock, index}) {
            axios.post('/api/watchlist/remove-idx-stock', {
                id: stock.id
            }).then((response) => {
                if (response.data.status) {
